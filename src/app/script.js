...
═══════════════════════════════════════════ */
function revealApp() {
  console.log('🚀 Shifster: Reveal App called');
...
async function bootApp() {
  ...
  if (!hasIndustry && !state.demoMode && window.location.pathname.includes('/app')) {
    showOnboarding();
  }
...
function updateHoursPreview() {
  ...
  if (!totalHours || totalHours <= 0) {
    ...
    if (preview) preview.textContent = fmt(netHours);
    if (badge) badge.classList.add('has-value');
...
async function saveHours() {
  ...
  const finalRate = rateInputVal || state.profile?.hourly_rate || DEFAULT_RATE;
  ...
  try {
    ...
    let netHours = totalHours;
    if (breakMin > 0 && breakIsPaid === false) {
      netHours = Math.max(0, totalHours - (breakMin / 60));
    }

    const parentId = state.modalParentId || null;
    await dbSetEntry(dk, netHours, finalRate, 'normal', startVal, endVal, targetStatus, totalHours, breakMin, breakIsPaid, isNight, parentId, state.modalEditingId || null, templateName, currency);

    closeHourModal();
    refreshAll();
  } catch (e) {
    console.error(e);
    toast('Грешка при записването на часовете', 'error');
  } finally {
    saveBtn.disabled = false;
    saveBtn.innerHTML = '✓ Запази смяната';
  }
}
...
═══════════════════════════════════════════ */
async function init() {
  console.log('🏁 Shifster: Init sequence started. Path:', window.location.pathname);
